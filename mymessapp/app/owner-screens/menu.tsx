...
export default function WeeklyMenuScreen() {
    ...
    const getUserData = async () => {
      try {
        ...
        if (messOwnerData) {
          ...
          if (email) {
            console.log('Using mess owner email:', email);
            setMessEmail(email);
        ...
        // If no mess owner data, try getting from route params
        ...
        // Last resort: use hardcoded test email to avoid the error
        const defaultEmail = 'shivbabanbale123@gmail.com';
        console.log('Using default mess owner email:', defaultEmail);
        setMessEmail(defaultEmail);
        checkMenuExists(defaultEmail);
        
      } catch (error) {
        console.error('Error getting mess owner data:', error);
        
        // Even in case of error, use the hardcoded email
        const defaultEmail = 'shivbabanbale123@gmail.com';
        console.log('Using default mess owner email after error:', defaultEmail);
        setMessEmail(defaultEmail);
        checkMenuExists(defaultEmail);
      }
    };
  ...
  const saveMenuData = async () => {
    ...
    if (!messEmail) {
      ...
      // Just use the hardcoded email directly
      const defaultEmail = 'shivbabanbale123@gmail.com';
      console.log('Using default mess owner email for saving:', defaultEmail);
      setMessEmail(defaultEmail);
      
      // Try again with the default email after a short delay
    ...
    try {
      ...
      // First show an Alert to confirm data is being prepared
      Alert.alert('Saving', 'Preparing to save your menu...');
        ...
        try {
          ...
          const navigateToHome = () => {
            ...
            try {
              ...
              try {
                // Fallback to direct navigation to owner home
                router.navigate('/(owner)');
                console.log('Navigated to owner home instead');
              } catch (homeError) {
          ...
          // Show success alert and navigate back to owner home screen
          Alert.alert(
            'Success', 
          ...
          const navigateToHome = () => {
            ...
            try {
              ...
              try {
                router.back();
                console.log('Navigated back instead');
              } catch (backError) {
                console.error('Back navigation error:', backError);
              }
            }
          };
          
          // Still show success if response was OK but couldn't parse JSON
          Alert.alert(
            'Success', 
            'Menu updated successfully',
            [{ 
